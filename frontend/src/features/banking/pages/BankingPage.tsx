       ...
       <div className="grx-glass-card rounded-xl overflow-hidden">
          <table className="w-full text-sm text-left">
             ...
             <tbody className="divide-y divide-grx-primary-50 dark:divide-grx-primary-800 bg-white dark:bg-grx-dark-surface">
                {transactions.map(t => (
                   <tr key={t.id} className="hover:bg-grx-bg dark:hover:bg-grx-primary-800">
                      <td className="px-6 py-4 text-grx-muted dark:text-grx-muted">{t.date}</td>
                      <td className="px-6 py-4 font-medium text-grx-text dark:text-white">{t.description}</td>
                      ...
                      <td className="px-6 py-4">
                         <span className={`px-2 py-1 rounded-full text-xs font-medium ${t.status === 'Reconciled' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : 'bg-grx-primary-50 dark:bg-grx-primary-800 text-grx-muted dark:text-grx-primary-200'}`}>
                            {t.status}
                         </span>
                      </td>
                      <td className="px-6 py-4">
