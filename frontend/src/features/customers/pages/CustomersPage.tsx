  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {customers.map(cust => (
          <div key={cust.id} className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 hover:shadow-md transition-shadow">
             <div className="flex justify-between items-start mb-4">
                ...
                <div className="text-right">
                   ...
                   <p className="font-bold text-slate-800 dark:text-slate-100">₹{cust.balance.toLocaleString('en-IN')}</p>
                </div>
             </div>
             <h3 className="font-semibold text-lg text-slate-800 dark:text-slate-100 mb-1">{cust.name}</h3>
             <p className="text-xs text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded w-fit mb-4">GSTIN: {cust.gstin}</p>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 w-full max-w-md shadow-2xl">
             <h3 className="text-lg font-bold mb-4 text-slate-900 dark:text-slate-100">New Customer</h3>
             <div className="space-y-3">
               <input placeholder="Company Name" className="w-full border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 p-2 rounded" value={newCust.name} onChange={e => setNewCust({...newCust, name: e.target.value})} />
             ...
             <div className="flex justify-end gap-3 mt-6">
               <button onClick={() => setShowAddModal(false)} className="text-slate-500 dark:text-slate-400 font-medium">Cancel</button>
