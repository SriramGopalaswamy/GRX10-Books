          ...
          <div key={cust.id} className="bg-white dark:bg-grx-dark-surface p-6 rounded-xl shadow-sm border border-grx-primary-100 dark:border-grx-primary-800 hover:shadow-md transition-shadow">
             <div className="flex justify-between items-start mb-4">
                ...
                <div className="text-right">
                   <p className="text-xs text-grx-muted">Outstanding</p>
                   <p className="font-bold text-grx-text dark:text-white">₹{cust.balance.toLocaleString('en-IN')}</p>
                </div>
             </div>
             <h3 className="font-semibold text-lg text-grx-text dark:text-white mb-1">{cust.name}</h3>
             <p className="text-xs text-grx-muted dark:text-grx-muted bg-grx-primary-50 dark:bg-grx-primary-800 px-2 py-0.5 rounded w-fit mb-4">GSTIN: {cust.gstin}</p>
             
             <div className="space-y-2 text-sm text-grx-muted dark:text-grx-primary-200">
                <div className="flex items-center gap-2"><Mail size={14} className="text-grx-muted"/> {cust.email}</div>
                {cust.phone && <div className="flex items-center gap-2"><Phone size={14} className="text-grx-muted"/> {cust.phone}</div>}
                {cust.address && <div className="flex items-center gap-2"><MapPin size={14} className="text-grx-muted"/> {cust.address}</div>}
             </div>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white dark:bg-grx-dark-surface rounded-xl p-6 w-full max-w-md shadow-2xl">
             ...
             <div className="flex justify-end gap-3 mt-6">
               <button onClick={() => setShowAddModal(false)} className="text-grx-muted dark:text-grx-muted font-medium">Cancel</button>
